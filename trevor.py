...
╚══════════════════════════════════════════════════════════════╝
        ...
        # SSL/TLS cipher cocok untuk di cek
        self.cipher_suites = {
...
🐞 /bug                   - Report bug problem on trevor to HMZZZ233
🌟 /ratings               - Rating this bot 1-10 v.2.6
...
⚠️  WARNING: Only use on systems you own or have explicit permission to test!
    ...
    def sql_injection_test(self, url):
        ...
        try:
            ...
            self.log_attack("sql_injection", url)
    ...
    def bruteforce_login(self, url):
        ...
        try:
            ...
            self.log_attack("bruteforce", url)
        except Exception as e:
    ...
    def fast_port_scan(self, host):
        ...
        if open_ports:
            ...
            self.log_attack("port_scan:fast", host)
    ...
    def full_port_scan(self, host):
        ...
        if open_ports:
            ...
            self.log_attack("port_scan:full", host)
            # Check for vulnerable services
            self.check_vulnerable_services(host, open_ports)
    ...
    def service_detection_scan(self, host):
        ...
        if open_ports:
            ...
            self.log_attack("port_scan:service", host)
            # Check for vulnerable services
            self.check_vulnerable_services(host, open_ports)
        self.log_attack("port_scan_services", host)
    ...
    def udp_port_scan(self, host):
        ...
        if open_ports:
            ...
            self.log_attack("port_scan:udp", host)
        ...
        try:
            ...
            if host in nm.all_hosts():
                ...
                if 'osmatch' in host_info:
                    ...
                    self.log_attack("port_scan:os", host)
                else:
    ...
    def dos_attack(self, target_url):
        ...
        try:
            ...
            self.log_attack("dos", target_url)
    ...
    def bug_report(self):
        ...
        print("\n🐞 BUG REPORT")
        print("Pilih jenis bug:")
        print("1. Sering error")
        print("2. Lambat")
        print("3. Ga efek ke URL target")
        print("4. Crash / force close")
        print("5. Performanya makin turun")
        print("6. Tulis sendiri (custom)")

        pilihan = input("Pilih (1-6): ")
        opsi = {
            "1": "Sering error",
            "2": "Lambat",
            "3": "Ga efek ke URL target",
            "4": "Crash / force close",
            "5": "Performanya makin turun"
        }

        if pilihan == "6":
            isi_bug = input("Tulis deskripsi bug: ")
        elif pilihan in opsi:
            isi_bug = opsi[pilihan]
        else:
        ...
        data.setdefault("bugs", []).append(bug_entry)

        with open("dashboard_data.json", "w") as f:
            json.dump(data, f, indent=2)

        print("✅ Bug report berhasil dikirim.")

    def ratings_report(self):
        import json
        from datetime import datetime

        print("\n⭐ RATING BOT")
        try:
            rating = int(input("Masukkan rating untuk Trevor Bot (1-10): "))
            if not 1 <= rating <= 10:
                print("❌ Rating harus antara 1 sampai 10.")
                return
        except:
        ...
        data.setdefault("ratings", []).append(rating_entry)

        with open("dashboard_data.json", "w") as f:
            json.dump(data, f, indent=2)

        print("✅ Rating berhasil dikirim.")
    ...
    def log_attack(self, attack_type, target_url):
        import json
        from datetime import datetime
        import requests
        ...
        data.setdefault("attacks", []).append(attack_entry)
...
def is_user_banned(username):
    ...
    c = conn.cursor()
    c.execute("SELECT * FROM banned_users WHERE username=?", (username,))
    result = c.fetchone()
    conn.close()
    return result is not None
...
def login_menu():
    ...
    res = requests.post(f"{SERVER}/api/login", json={"username": username, "password": password})
    ...
    if status == "ok":
        ...
        choice = input("➕ Register this user? (y/n): ").lower()
        if choice == "y":
            reg = requests.post(f"{SERVER}/api/register", json={"username": username, "password": password})
            if reg.json().get("status") == "registered":
...
def check_banned(username):
    res = requests.post(f"{SERVER}/api/is_banned", json={"username": username})
    return res.json().get("banned", False)
            
if __name__ == "__main__":
    # ⛏️ Pastikan semua dependencies terinstall
    required_packages = ['requests', 'beautifulsoup4', 'flask', 'pyopenssl', 'cryptography', 'dnspython', 'python-nmap', 'scapy']
    ...
    # 🔐 Login atau daftar user
    print("\nTREVOR BOT LOGIN 2.6🚀\n")
    current_user = login_menu()
    if check_banned(current_user):
        print(f"🚫 Sorry, user '{current_user}' is banned!")
        sys.exit()
    ...
    # 🚀 Start bot
    bot = Trevor()
    bot.username = current_user  # inject username aktif
    bot.run()
